...
- cung cấp ứng dụng GUI desktop `usb_guard_gui` viết bằng C++ và GTK 3 với giao diện tiếng Việt;
- bổ sung thống kê lưu lượng đọc/ghi và tốc độ đọc/ghi của USB ở mức block layer trong ứng dụng GUI.
- cho phép thêm và xóa whitelist runtime thông qua giao diện điều khiển của module và ứng dụng GUI;
- cho phép kernel module tự động chặn USB ngoài whitelist bằng cách ghi `authorized=0` vào `sysfs` ngay sau khi nhận diện thiết bị;
- hỗ trợ chế độ tự động chặn USB ngoài whitelist trong giao diện giám sát.
...
- Các ứng dụng user space gồm `usb_guard_monitor` và `usb_guard_gui` sẽ đọc kết quả từ `/proc/usb_guard/status`, kết hợp thêm thông tin từ `sysfs` để hiển thị block device, dung lượng, mountpoint, trạng thái `authorized` và thống kê I/O ở mức block layer.
...
- Trong chế độ `whitelist`, thiết bị không nằm trong danh sách sẽ bị đánh dấu `UNTRUSTED` và sinh cảnh báo.
 ...
 Khi cần chặn thiết bị ngoài whitelist, `usb_guard` sẽ xếp một `workqueue` trong kernel rồi ghi `authorized=0` vào `/sys/bus/usb/devices/<devpath>/authorized`. Cách làm này vẫn giữ nguyên `usb-storage`, nhưng khiến thiết bị USB bị deauthorize nên hệ điều hành sẽ ngắt quyền sử dụng đối với thiết bị đó.
...
- `usb_guard_gui.cpp`: ứng dụng GUI desktop bằng C++ và GTK 3, giao diện tiếng Việt, hiển thị trạng thái USB, lịch sử, nhật ký, block device và I/O;
...
- ứng dụng GUI `usb_guard_gui` cung cấp bảng điều khiển tiếng Việt với phần tổng quan, danh sách USB đang kết nối, lịch sử và nhật ký;
- GUI hiển thị block device, mountpoint, lưu lượng đọc/ghi tổng và tốc độ đọc/ghi hiện tại;
- GUI cho phép đổi quyền truy cập theo từng USB đang chọn giữa hai chế độ `chỉ đọc` và `đọc/ghi`;
- GUI lưu bền vững whitelist và policy truy cập theo từng USB vào file cấu hình để tự nạp lại ở lần mở sau;
- giao diện GUI đã được tinh gọn để dễ quan sát hơn, với bảng thiết bị rút gọn, khung chi tiết đặt cạnh bên và hệ màu sáng đồng nhất;
- module tạo thêm giao diện điều khiển `/proc/usb_guard/control` cho phép thêm, xóa hoặc xóa toàn bộ whitelist trong lúc chạy;
- GUI có các nút `Thêm whitelist` và `Xóa whitelist` để quản lý rule trực tiếp từ cửa sổ ứng dụng;
- khi xóa whitelist, GUI hiển thị danh sách các rule hiện có để người dùng chọn đúng mục cần xóa;
- module hỗ trợ tham số `block_untrusted=1|0` và lệnh runtime `block on|off` để bật hoặc tắt cơ chế chặn ngay trong kernel module;
- tùy chọn `--deauthorize-untrusted` cho phép ứng dụng C++ ghi `authorized=0` để chặn USB ngoài whitelist;
- thao tác `chỉ đọc/đọc-ghi` được áp thật trên đúng USB được chọn bằng `BLKROSET` và remount mountpoint, không áp dụng đồng loạt cho mọi USB;
- khi mở lại GUI, cấu hình đã lưu có thể được đồng bộ lại vào kernel module và tái áp dụng cho USB phù hợp;
- trong `observe mode`, ứng dụng hiển thị nhãn `monitor-only` thay vì `UNTRUSTED`;
- khi `block_untrusted=1`, USB ngoài whitelist sẽ bị module xếp lịch deauthorize ngay sau sự kiện `USB_DEVICE_ADD`;
- sau khi bị deauthorize, thiết bị sẽ chuyển sang trạng thái `Đã chặn` trong giao diện;
- thông báo của ứng dụng phản ánh rõ kết quả xử lý thực tế ở phần CLI và trạng thái hiển thị trên giao diện.
...
#### 6.2.6. Kịch bản 6: chặn USB ngoài whitelist

Nạp module với `whitelist` và `block_untrusted=1`, hoặc bật thêm cơ chế chặn từ ứng dụng giám sát. Khi cắm một USB không nằm trong danh sách cho phép, kiểm tra:

- thiết bị bị ghi `authorized=0`;
- trạng thái trong giao diện chuyển sang `Đã chặn`;
- USB không còn được sử dụng bình thường sau khi bị deauthorize;
- nhật ký của ứng dụng ghi rõ thao tác chặn đã thực hiện và kết quả xử lý thực tế.

#### 6.2.7. Kịch bản 7: đặt quyền truy cập cho từng USB

Khi có nhiều USB đang kết nối, chọn một USB cụ thể trong GUI rồi thử hai thao tác:

- đặt thiết bị sang chế độ `chỉ đọc`;
- đặt thiết bị trở lại chế độ `đọc/ghi`.

Kết quả cần kiểm tra:

- chỉ USB đang chọn bị thay đổi quyền truy cập;
- các USB khác không bị ảnh hưởng;
- trạng thái `Truy cập` trong GUI đổi đúng sang `Chỉ đọc` hoặc `Đọc/ghi`;
- nếu thiết bị đang mount, mountpoint được remount tương ứng;
- việc thay đổi này là thay đổi thật, không phải mô phỏng.

#### 6.2.8. Kịch bản 8: khôi phục cấu hình sau khi tắt và mở lại GUI

Sau khi đã thêm whitelist hoặc đặt `chỉ đọc/đọc-ghi` cho một USB cụ thể, đóng GUI rồi mở lại. Kiểm tra:

- file cấu hình bền vững vẫn còn tồn tại;
- GUI đọc lại được whitelist và policy truy cập đã lưu;
- nếu GUI chạy bằng quyền `root`, whitelist được đồng bộ lại vào kernel module;
- nếu USB tương ứng đang cắm, policy `chỉ đọc/đọc-ghi` được áp lại đúng thiết bị.
...
- Nếu bật chính sách chặn, USB ngoài whitelist sẽ bị deauthorize và bị ngắt quyền sử dụng thực tế.
- GUI có thể đổi quyền truy cập `chỉ đọc/đọc-ghi` cho từng USB riêng lẻ.
- Whitelist và policy truy cập vẫn còn sau khi đóng và mở lại GUI.
...
- kernel module có thể chủ động deauthorize thiết bị ngoài whitelist;
- ứng dụng C++ hiển thị thông tin quản trị, thống kê và cung cấp thêm cơ chế điều khiển chính sách ở phía người dùng.
...
- Cơ chế chặn hiện tại vẫn dựa trên việc ghi `authorized=0` vào `sysfs`; module đã tự thực hiện bước này trong kernel, nhưng đây vẫn chưa phải là API deauthorize nội bộ được export trực tiếp từ USB core cho module ngoài cây kernel.
...
- thay cơ chế ghi `authorized=0` qua `sysfs` bằng một API deauthorize nội bộ của USB core nếu sau này chuyển dự án vào trong cây kernel hoặc có đủ symbol export.
...
dmesg | tail -n 50
lsmod | grep usb
sudo insmod ten_module.ko whitelist=346d:5678 block_untrusted=1
...
- Hình giao diện GUI `usb_guard_gui` với danh sách USB và tốc độ đọc/ghi.
